                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                    ...
                    <div className="mt-4 space-y-6">
                      ...
                      {(complaint.name || shouldMaskData) && (
                        <div>
                          <div className="text-sm text-gray-500 mb-1">Nome</div>
                          <div className="text-sm font-medium text-gray-900">
                            {shouldMaskData ? maskSensitiveData(complaint.name) : complaint.name}
                          </div>
                        </div>
                      )}

                      {(complaint.email || shouldMaskData) && (
                        <div>
                          <div className="text-sm text-gray-500 mb-1">E-mail</div>
                          <div className="text-sm font-medium text-gray-900">
                            {shouldMaskData ? maskSensitiveData(complaint.email, 12) : complaint.email}
                          </div>
                        </div>
                      )}
                      ...
                      {complaint.who_did_it && (
                        <div>
                          <div className="text-sm text-gray-500 mb-1">Quem fez isto?</div>
                          <div className="text-sm font-medium text-gray-900">{complaint.who_did_it}</div>
                        </div>
                      )}

                      {complaint.how_it_happened && (
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {shouldMaskData ? (
                              <div className="text-sm text-gray-500">
                                {complaint.attachments.split(',').length} anexo(s) oculto(s) por privacidade
                              </div>
                            ) : (
                              complaint.attachments.split(',').map((attachment, index) => (
                                <a
                                  key={index}
