...
try {
  db.prepare('ALTER TABLE users ADD COLUMN can_view_denuncias INTEGER NOT NULL DEFAULT 1').run();
} catch (e) {}
try {
  db.prepare('ALTER TABLE users ADD COLUMN can_view_documentacao INTEGER NOT NULL DEFAULT 1').run();
} catch (e) {}
try {
  db.prepare('ALTER TABLE users ADD COLUMN can_view_naoconformidades INTEGER NOT NULL DEFAULT 1').run();
} catch (e) {}
try {
  db.prepare('ALTER TABLE users ADD COLUMN can_view_empresas INTEGER NOT NULL DEFAULT 1').run();
} catch (e) {}
try {
...
try {
  db.prepare('ALTER TABLE users ADD COLUMN first_login_required INTEGER NOT NULL DEFAULT 0').run();
} catch (e) {}
...
if (count === 0) {
  ...
  const stmt = db.prepare('INSERT INTO events (date, category, company, status, description) VALUES (?, ?, ?, ?, ?)');
  const sampleData = [
    [
      '2024-03-16 13:57:41',
      'Cadastros',
...
const checkAdminUser = (req, res, next) => {
  try {
    ...
    const adminUser = db.prepare('SELECT * FROM users WHERE user_type = ? LIMIT 1').get('admin');
    if (!adminUser) {
      return res.status(403).json({ error: 'Acesso negado. Nenhum administrador encontrado.' });
    }
...
app.get('/api/events', (req, res) => {
  try {
    ...
    if (company) {
      ...
      events = db.prepare('SELECT * FROM events WHERE company = ? ORDER BY date DESC').all(company);
    } else {
...
app.post('/api/events', checkAdminUser, (req, res) => {
  try {
    ...
    console.log('Dados recebidos:', { date, category, company, status, description });
    ...
    if (category === 'Denúncias') {
      ...
      const companyData = db.prepare('SELECT id, slug FROM companies WHERE name = ?').get(company);
      if (!companyData) {
...
app.put('/api/events/:id', (req, res) => {
  try {
    ...
    console.log('Dados recebidos para atualização:', { id, category, company, status, description });
...
app.get('/api/stats', (req, res) => {
  try {
    ...
    if (company) {
      // Se uma empresa específica foi solicitada
      stats = {
        total: db.prepare('SELECT COUNT(*) as count FROM events WHERE company = ?').get(company).count,
        open: db.prepare('SELECT COUNT(*) as count FROM events WHERE status = ? AND company = ?').get('aberto', company).count,
        closed: db.prepare('SELECT COUNT(*) as count FROM events WHERE status = ? AND company = ?').get('fechado', company).count,
        byCategory: db.prepare('SELECT category, COUNT(*) as count FROM events WHERE company = ? GROUP BY category').all(company)
      };
    } else {
      ...
      stats = {
        ...
        open: db.prepare('SELECT COUNT(*) as count FROM events WHERE status = ?').get('aberto').count,
        closed: db.prepare('SELECT COUNT(*) as count FROM events WHERE status = ?').get('fechado').count,
        byCategory: db.prepare('SELECT category, COUNT(*) as count FROM events GROUP BY category').all()
      };
    }
...
app.get('/api/nonconformities', (req, res) => {
  try {
    ...
    if (company) {
      // Se uma empresa específica foi solicitada
      const companyData = db.prepare('SELECT id FROM companies WHERE name = ?').get(company);
      if (!companyData) {
        return res.status(404).json({ error: 'Empresa não encontrada' });
      }
      nonconformities = db.prepare(`
...
app.post('/api/nonconformities', upload.array('attachments'), (req, res) => {
  try {
    const { name, email, subject, description, whoDidIt, howItHappened, additionalInfo, setor, company_id } = req.body;
    ...
    const newNonconformity = db.prepare(`
      SELECT n.*, c.name as company_name, c.slug as company_slug
      FROM nonconformities n
      JOIN companies c ON n.company_id = c.id
      WHERE n.id = ?
    ...
    res.status(500).json({ error: 'Erro ao criar não conformidade', details: error.message });
  }
});
...
app.post('/api/companies', upload.single('logo'), (req, res) => {
  try {
    ...
    const { name, cnpj, phone, email, address, primaryColor, secondaryColor } = req.body;
    ...
    const existingCompany = db.prepare('SELECT id FROM companies WHERE slug = ?').get(slug);
    if (existingCompany) {
    ...
    const newCompany = db.prepare('SELECT * FROM companies WHERE id = ?').get(transaction());
...
app.delete('/api/companies/:slug', (req, res) => {
  try {
    ...
    // Verificar se a empresa existe
    const company = db.prepare('SELECT * FROM companies WHERE slug = ? AND id = ?').get(slug, id);
    if (!company) {
      return res.status(404).json({ error: 'Empresa não encontrada' });
    }
    ...
    const transaction = db.transaction(() => {
      ...
      db.prepare('DELETE FROM nonconformities WHERE company_id = ?').run(company.id);
      
      // Excluir usuários relacionados
      db.prepare('DELETE FROM users WHERE company_id = ?').run(company.id);
      
      // Por fim, excluir a empresa
      const result = db.prepare('DELETE FROM companies WHERE id = ?').run(company.id);
...
app.post('/api/companies/:slug/complaints', upload.array('attachments'), (req, res) => {
  try {
    const { slug } = req.params;
    const { name, email, subject, description, whoDidIt, howItHappened, additionalInfo } = req.body;
...
app.post('/api/companies/:slug/nonconformities', upload.array('attachments'), (req, res) => {
  try {
    const { slug } = req.params;
    const { name, email, subject, description, whoDidIt, howItHappened, additionalInfo, setor } = req.body;
    ...
    // Buscar a não conformidade recém-criada
    const newNonconformity = db.prepare('SELECT * FROM nonconformities WHERE id = ?').get(result.lastInsertRowid);
    res.status(201).json(newNonconformity);
  } catch (error) {
    console.error('Erro ao criar não conformidade:', error);
    res.status(500).json({ error: 'Erro ao criar não conformidade', details: error.message });
...
app.get('/api/companies/:slug/nonconformities', (req, res) => {
  try {
    ...
    const nonconformities = db.prepare(`
    ...
    console.log(`Não conformidades encontradas para ${slug}:`, nonconformities.length);
    res.json(nonconformities);
  } catch (error) {
    console.error('Erro ao buscar não conformidades:', error);
    res.status(500).json({ error: 'Erro ao buscar não conformidades' });
...
app.put('/api/companies/:id/status', (req, res) => {
  try {
    ...
    eventStmt.run(
      ...
      company.name,
      'fechado',
      `Status da empresa alterado para ${status === 'active' ? 'Ativo' : 'Inativo'}`
    );
...
app.put('/api/users/:id/password', (req, res) => {
  try {
    ...
    const updatedUser = db.prepare('SELECT id, username, password FROM users WHERE id = ?').get(id);
...
app.put('/api/users/:id/permissions', (req, res) => {
  try {
    const { id } = req.params;
    const { can_view_denuncias, can_view_documentacao, can_view_naoconformidades, can_view_empresas } = req.body;
    // Apenas admin pode alterar
    // (implemente autenticação real em produção)
    const stmt = db.prepare('UPDATE users SET can_view_denuncias = ?, can_view_documentacao = ?, can_view_naoconformidades = ?, can_view_empresas = ? WHERE id = ?');
    const result = stmt.run(
    ...
    const updatedUser = db.prepare('SELECT id, username, can_view_denuncias, can_view_documentacao, can_view_naoconformidades, can_view_empresas FROM users WHERE id = ?').get(id);
...
app.post('/api/users', /* checkAdminUser, */ (req, res) => {
  try {
    const { 
      username, 
      password, 
    ...
    if (!username || !password || !full_name || !company_id) {
      return res.status(400).json({ error: 'Campos obrigatórios: username, password, full_name, company_id' });
    }
    ...
    if (full_name.length < 2 || full_name.length > 100) {
      return res.status(400).json({ error: 'Nome completo deve ter entre 2 e 100 caracteres' });
    }

    // Validar username (apenas letras, números, pontos e underscores)
    if (!/^[a-zA-Z0-9._]+$/.test(username)) {
      return res.status(400).json({ error: 'Username deve conter apenas letras, números, pontos e underscores' });
    }
    ...
    if (department_id) {
      const dept = db.prepare('SELECT id FROM departments WHERE id = ? AND company_id = ?').get(department_id, company_id);
      if (!dept) {
        return res.status(400).json({ error: 'Departamento não encontrado ou não pertence à empresa' });
      }
    }

    // Verificar se cargo existe (se informado)
    if (role_id) {
      const role = db.prepare('SELECT id FROM roles WHERE id = ? AND company_id = ?').get(role_id, company_id);
      if (!role) {
        return res.status(400).json({ error: 'Cargo não encontrado ou não pertence à empresa' });
      }
    }

    // Verificar se gerente existe (se informado)
    if (manager_id) {
      const manager = db.prepare('SELECT id FROM users WHERE id = ? AND company_id = ? AND status = "active"').get(manager_id, company_id);
      if (!manager) {
        return res.status(400).json({ error: 'Gerente não encontrado ou inativo' });
      }
    }

    // Verificar se código de funcionário já existe na empresa (se informado)
    if (employee_code) {
      const existingCode = db.prepare('SELECT id FROM users WHERE employee_code = ? AND company_id = ?').get(employee_code, company_id);
      if (existingCode) {
        return res.status(400).json({ error: 'Código de funcionário já existe nesta empresa' });
      }
    }
    ...
    console.log('Funcionário criado:', newUser.full_name, 'para empresa:', company.name);
...
const pathGoogleKey = path.join(__dirname, '../../google-service-account.json');
let driveAuth = null;
let driveService = null;
try {
  driveAuth = new google.auth.GoogleAuth({
    keyFile: pathGoogleKey,
    scopes: ['https://www.googleapis.com/auth/drive.readonly'],
  });
  driveService = google.drive({ version: 'v3', auth: driveAuth });
} catch (e) {
  console.error('Erro ao configurar Google Drive:', e);
}
...
(async () => {
  try {
    if (driveAuth) {
      ...
      console.log('[DRIVE TEST] Resultado da busca de pastas:', folderList.data.files);
      ...
      console.log('[DRIVE TEST] Arquivos encontrados na pasta ALLEG:', files.data.files);
    }
  } catch (err) {
...
app.get('/api/documentacao/drive', async (req, res) => {
  try {
    ...
    if (folderId) {
      ...
      if (!folderList.data.files.length) {
        return res.status(404).json({ 
          ...
          details: 'Verifique se a pasta existe no Google Drive e se as permissões estão configuradas corretamente'
        });
      }
    ...
    // Log para debug
    console.log(`[DRIVE] Listando arquivos da pasta ${parentFolderId}:`, files.data.files.length);
...
app.get('/api/documentacao/drive-debug', async (req, res) => {
  try {
    ...
    console.log('[DRIVE DEBUG] Resultado da busca de pastas:', folderList.data.files);
    ...
    console.log('[DRIVE DEBUG] Arquivos encontrados na pasta ALLEG:', files.data.files);
    ...
    console.error('[DRIVE DEBUG] Erro detalhado ao listar arquivos do Drive:', error);
    res.status(500).json({ 
...
app.post('/api/chat/messages', (req, res) => {
  try {
    ...
    const newMessage = db.prepare('SELECT * FROM chat_messages WHERE id = ?').get(result.lastInsertRowid);
...
app.post('/api/chat/messages/:id/like', (req, res) => {
  try {
    ...
    const existingLike = db.prepare('SELECT id FROM chat_message_likes WHERE message_id = ? AND user_id = ?').get(id, user_id);
    
    if (existingLike) {
      // Remover like
      db.prepare('DELETE FROM chat_message_likes WHERE message_id = ? AND user_id = ?').run(id, user_id);
      db.prepare('UPDATE chat_messages SET likes = likes - 1 WHERE id = ?').run(id);
    } else {
      // Adicionar like
      db.prepare('INSERT INTO chat_message_likes (message_id, user_id) VALUES (?, ?)').run(id, user_id);
      db.prepare('UPDATE chat_messages SET likes = likes + 1 WHERE id = ?').run(id);
    }
    
    const updatedMessage = db.prepare('SELECT * FROM chat_messages WHERE id = ?').get(id);
...
app.post('/api/departments', checkAdminUser, (req, res) => {
  try {
    const { company_id, name, code, description, parent_id, manager_user_id } = req.body;
    
    // Validações
    if (!company_id || !name || !code) {
      return res.status(400).json({ error: 'Campos obrigatórios: company_id, name, code' });
    }
    ...
    if (!/^[A-Z0-9_]+$/.test(code)) {
      return res.status(400).json({ error: 'Código deve conter apenas letras maiúsculas, números e underscore' });
    }
    
    // Verificar se código já existe na empresa
    const existingCode = db.prepare('SELECT id FROM departments WHERE company_id = ? AND code = ?').get(company_id, code);
    if (existingCode) {
    ...
    // Verificar se gerente existe (se informado)
    if (manager_user_id) {
      const manager = db.prepare('SELECT id FROM users WHERE id = ? AND company_id = ? AND status = "active"').get(manager_user_id, company_id);
      if (!manager) {
        return res.status(400).json({ error: 'Gerente não encontrado ou inativo' });
      }
    }
    ...
    const result = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
    `).run(company_id, name, code, description, parent_id || null, manager_user_id || null);
...
app.put('/api/departments/:id', checkAdminUser, (req, res) => {
  try {
    const { id } = req.params;
    const { name, code, description, parent_id, manager_user_id, is_active } = req.body;
    ...
    if (code && code !== department.code) {
      ...
      const existingCode = db.prepare('SELECT id FROM departments WHERE company_id = ? AND code = ? AND id != ?').get(department.company_id, code, id);
      if (existingCode) {
        return res.status(400).json({ error: 'Código já existe nesta empresa' });
      }
    }
    ...
    `).run(
      ...
      manager_user_id !== undefined ? manager_user_id : department.manager_user_id,
...
app.post('/api/roles', checkAdminUser, (req, res) => {
  try {
    ...
    // Verificar se código já existe na empresa
    const existingCode = db.prepare('SELECT id FROM roles WHERE company_id = ? AND code = ?').get(company_id, code);
    if (existingCode) {
      return res.status(400).json({ error: 'Código já existe nesta empresa' });
    }
...
app.post('/api/users/:id/permissions', checkAdminUser, (req, res) => {
  try {
    ...
    const permission = db.prepare('SELECT id FROM permissions WHERE id = ?').get(permission_id);
    if (!permission) {
    ...
    const result = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, datetime('now'), ?, ?)
    `).run(id, permission_id, department_id || null, 1, expires_at || null, notes || null);
...
app.get('/api/indicators', (req, res) => {
  try {
    const indicators = db.prepare('SELECT * FROM indicators ORDER BY name').all();
    res.json(indicators);
  } catch (error) {
...
app.put('/api/indicators/:id', checkAdminUser, (req, res) => {
  try {
    ...
    if (current_value === undefined || target_value === undefined) {
      return res.status(400).json({ error: 'Valores atual e meta são obrigatórios' });
    }
    ...
    const updatedIndicator = db.prepare('SELECT * FROM indicators WHERE id = ?').get(id);
...
const server = app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('Arquivo do banco:', path.resolve('events.db'));
});
